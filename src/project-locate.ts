 ...
 */
export function locateProjectDir(cwd: string, name?: string): string {
  if (!name) {
    if (hasGit(cwd)) {
      return cwd;
    }
    throw new Error(
      `"${cwd}" não é um repositório git e nenhum nome de projeto foi informado — rode de dentro do projeto ou passe "aleksandria draft <projeto>".`,
    );
  }
